...
def create_pertoken_html(all_models_results, selected_steps=None, model_name="", dataset_name=""):
    ...
    if default_context:
        html += create_single_context_raw_html(results, default_context, selected_steps)
    else:
...
def create_toggle_script(all_models_results, context_indices, 
    ...
    <script>
        ...
        function createSingleContextStepDiffHtml(results, contextId, selectedSteps) {{
            ...
            // Calculate all loss differences to find normalization factor
            let allDiffs = [];
            for (let i = 1; i < steps.length; i++) {{
                const prevStep = steps[i-1];
                const currStep = steps[i];
                const prevLosses = contextData["checkpoints"][prevStep]["losses"];
                const currLosses = contextData["checkpoints"][currStep]["losses"];
                
                if (!prevLosses || !currLosses) continue;
                
                for (let j = 0; j < Math.min(prevLosses.length, currLosses.length); j++) {{
                    // Skip if either value is null
                    if (prevLosses[j] === null || currLosses[j] === null) continue;
                    const diff = currLosses[j] - prevLosses[j];
                    allDiffs.push(Math.abs(diff));
                }}
            }}
            
            // Sort diffs and remove insignificant ones for meaningful stats
...
def create_single_context_raw_html(results, context_idx, selected_steps):
    """
